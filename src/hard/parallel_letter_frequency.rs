...
pub fn frequency(input: &[&str], worker_count: usize) -> HashMap<char, usize> {
    ...
    let shared_map = Arc::new(Mutex::new(HashMap::<char, usize>::new()));
    let mut handles = vec![];
    ...
    for chunk in input.chunks(chunk_size) {
        ...
        handles.push(thread::spawn(move || {
            ...
            let mut global_map = map_clone.lock().unwrap();
            for (c, count) in local_map {
                *global_map.entry(c).or_insert(0) += count;
            }
        }));
    }
    ...
    Arc::try_unwrap(shared_map).unwrap().into_inner().unwrap()
}
...
mod tests {
    ...
    fn one_text_with_multiple_letters() {
        ...
        hm.insert('c', 3);
        hm.insert('d', 1);
        assert_eq!(frequency(&["bbcccd"], 4), hm);
    }
    ...
    fn two_texts_with_multiple_letters() {
        ...
        hm.insert('h', 3);
        hm.insert('i', 1);
        assert_eq!(frequency(&["ggh", "hhi"], 4), hm);
    }
    ...
    fn large_texts() {
        ...
        .into_iter()
        .collect();
        assert_eq!(frequency(&DOSTOEVSKY, 4), expected);
    }
