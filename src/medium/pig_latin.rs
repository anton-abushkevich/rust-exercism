...
pub fn translate(input: &str) -> String {
    input
        ...
        .map(|w| {
            // Rule 1
            if w.starts_with("xr") || w.starts_with("yt") ||
                w.chars().next().is_some_and(|c| VOWELS.contains(c)) {
                return format!("{}ay", w);
            }
            ...
            while let Some(&c) = chars.peek() {          
                if !consonants.is_empty() && c == 'y' { // Rule 4
                    break;
                } 
                if !VOWELS.contains(c) { // Rule 2
                    consonants.push(c);
                    chars.next();
                    if c == 'q' && chars.peek() == Some(&'u') { // Rule 3
                        consonants.push('u');
                        chars.next();
                    }                    
                } else {
